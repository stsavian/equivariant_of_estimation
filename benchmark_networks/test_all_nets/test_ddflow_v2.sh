...
do
   case "$opt" in
      ...
      f ) fake_input_size="$OPTARG" ;;
      ? ) helpFunction ;; # Print helpFunction in case parameter is non-existent
   esac
done

# Print helpFunction in case parameters are empty
if [ -z "$model_name" ] || [ -z "$model_path" ] || [ -z "$destination_folder" ] || [ -z "$testing_datasets" ]|| [ -z "$test_mean_O_2" ] || [ -z "$fake_input_size" ]
then
...
/home/ssavian/anaconda3/envs/selflow/bin/python test_ddflow.py \
...
$matlab_equivariance_pth  --monkaa_pth $monkaa_path --hd1k_pth $hd1k_path  --kitti_pth $kitti_path --testing_datasets $testing_datasets $test_mean --fake_input_size $fake_input_size
