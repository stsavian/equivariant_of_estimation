...
def main(args):
    ...
    dest_path=os.path.join(dest_path,'full_frame_hist')
    if not os.path.exists(dest_path): #be careful if dest path match the folder
        os.makedirs(dest_path)
    if args.L2_eval:
        dest_path = os.path.join(dest_path, 'L2_hist')
    else:
    ...
    #######
    full_frame_dataset_list = list(filter(lambda x: re.search('full_frame', x) != None, folders_names_list))
    for item in full_frame_dataset_list:
        ...
        full_frame_reduced_csv = list(filter(lambda x: re.search('reduced', x) != None, full_frame_csv_list))
        print(full_frame_csv_pth)
        transforms_reduced_csv_pth = os.path.join(full_frame_csv_pth, full_frame_reduced_csv[0])
        read_csv_and_plot(transforms_reduced_csv_pth, dest_path,full_frame_reduced_csv[0][:-4])
