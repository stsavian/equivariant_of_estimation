...
def read_csv_and_plot(data_pth,dest_path,plot_name):
    ...
    assert data.columns.to_list() == ['Iu_T180_m1', 'Iu_Tlr_m1', 'Iv_T180_m1', 'Iv_Tud_m1']
    ...
    dataset_labels = ['AC', 'AC', 'AC', 'BT', 'ACo', 'DK', 'CS', 'BT', 'AC', 'DK', 'CS', 'BT', 'BT', 'AC', 'AC', 'AC',
                      'BT', 'BT', 'BT', 'DK', 'DK', 'AC2-FWD', 'AC2-ALL', 'CS', 'CS', 'BT-FWD', 'BT2']
    ...
    data_u= data[['Iu(T180)', 'Iu(TLR)', 'dataset']]
    data_v = data[['Iv(T180)', 'Iv(TUD)', 'dataset']]
    for data,label in zip([data_u,data_v],['u','v']):
        fig, ax = plt.subplots(figsize=(10, 7))
        if label=='u':
            data.plot.bar(ax=ax, width=0.9, alpha=0.6, legend=True, color=['blue', 'skyblue', 'r', 'coral'])
        if label=='v':
            data.plot.bar(ax=ax, width=0.9, alpha=0.6, legend=True, color=['r', 'coral','blue', 'skyblue'])
        ...
        ax.set_ylabel('Imbalance '+label+' [px]', color='#333F4B')
        ...
        fig.tight_layout()
        if len(data)<15:
            fig.savefig(os.path.join(dest_path, plot_name+'_'+label+'_raft_train'+'.pdf'), format='pdf', pad_inches=0)
        else:
            fig.savefig(os.path.join(dest_path, plot_name + '_' + label + '.pdf'), format='pdf',
                        pad_inches=0)
...
def main(args):
    ...
    dest_path=os.path.join(dest_path,'transform_hist')
    if not os.path.exists(dest_path): #be careful if dest path match the folder
        os.makedirs(dest_path)
    ...
    #######
    test_transforms_dataset_list = list(filter(lambda x: re.search('test_transforms', x) != None, folders_names_list))
    for item in test_transforms_dataset_list:
        transforms_mat_csv_pth = os.path.join(root_path, item)
        transforms_csv_list = os.listdir(transforms_mat_csv_pth)
        transforms_reduced_csv = list(filter(lambda x: re.search('reduced', x) != None, transforms_csv_list))

        print(transforms_mat_csv_pth)
        print(transforms_reduced_csv[0])
        transforms_csv_pth = os.path.join(transforms_mat_csv_pth, transforms_reduced_csv[0])

        read_csv_and_plot(transforms_csv_pth, dest_path,transforms_reduced_csv[0][:-4])
