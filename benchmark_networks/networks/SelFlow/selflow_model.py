...
from networks.SelFlow.network import pyramid_processing
from networks.SelFlow.datasets import BasicDataset
from networks.SelFlow.utils import average_gradients, lrelu, occlusion, rgb_bgr
from networks.SelFlow.data_augmentation import flow_resize
from networks.SelFlow.flowlib import flow_to_color, write_flo
...
class SelFlowModel(object):
    ...
    def test(self, restore_model, save_dir, is_normalize_img=True):
        ...
        for i in range(dataset.data_num):
            #save_dir = '/home/ssavian/training/plots/'
            ...
            imwrite('%s/flow_fw_color_%s.png' % (save_dir, save_name_list[i]), np_flow_fw_color[0])
            imwrite('%s/flow_bw_color_%s.png' % (save_dir, save_name_list[i]), np_flow_bw_color[0])
            ...
            #imwrite(os.path.join('/home/ssavian/training/plots/selfow_test.png'),np_flow_fw_color[0])

            # imwrite( os.path.join(save_dir, save_name_list[i]), np_flow_bw_color[0])
            # write_flo( (save_dir, save_name_list[i]), np_flow_fw[0])
            # write_flo( (save_dir, save_name_list[i]), np_flow_bw[0])
